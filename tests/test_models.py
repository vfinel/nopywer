...
def test_node_to_geojson_emits_phase():
    """`to_geojson` includes the `phase` property for any non-None form.

    What: an int phase and a multi-phase list appear in the exported
    properties; only `phase=None` is omitted.
